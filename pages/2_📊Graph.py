...
def num_summary(data, var):
    ...
    st.write(f"{'Data Type':15}: {col.dtype}")
    st.write(f"{'Missing Data':15}: {col.isna().sum():,} rows ({col.isna().mean() * 100:.2f} %)")
    st.write(f"{'Available Data':15}: {col.count():,} / {len(col):,} rows")
    ...
    alpha = 0.05
    # test for normality
    st.markdown("### Hypothesis Testing for Normality:")
    # Shapiro-Wilk test
    ...
    st.write(f"{'Significance Level':21}: {alpha}")
    st.write(f"{'Null Hypothesis':21}: The data is normally distributed")
    st.write(f"{'Alternate Hypothesis':21}: The data is not normally distributed")
    st.write(f"{'p-value':21}: {sw_pvalue}")
    st.write(f"{'Test Statistic':21}: {sw_statistic}")
    if sw_pvalue < alpha:
        st.write(f"- Since p-value is less than alpha ({alpha}), we Reject the Null Hypothesis at {alpha * 100}% significance level")
        st.write("- **CONCLUSION**: We conclude that the data sample is not normally distributed")
    else:
        st.write(f"- Since p-value is greater than alpha ({alpha}), we Fail to Reject the Null Hypothesis at {alpha * 100}% significance level")
        st.write("- **CONCLUSION**: We conclude that the data sample is normally distributed")
    ...
    st.write(f"{'Null Hypothesis':21}: The data is normally distributed")
    st.write(f"{'Alternate Hypothesis':21}: The data is not normally distributed")
    st.write(f"{'Critical Value':21}: {ad_critical}")
    st.write(f"{'Test Statistic':21}: {ad_statistic}")
    if ad_statistic >= ad_critical:
        st.write(f"- Since the Test-statistic is greater than Critical Value, we Reject the Null Hypothesis at {alpha * 100}% significance level")
        st.write("- **CONCLUSION**: We conclude that the data sample is not normally distributed")
    else:
        st.write(f"- Since the Test-statistic is less than Critical Value, we Fail to Reject the Null Hypothesis at {alpha * 100}% significance level")
        st.write("- **CONCLUSION**: We conclude that the data sample is normally distributed")

# hypothesis testing for association between 2 numeric variables
def num_num_hyp_testing(data, var1, var2, alpha=0.05):
    st.markdown(f"## Hypothesis Test for Association between {var1} and {var2}")
    ...
    st.markdown("### Pearson Test")
    st.write(f"- {'Significance Level':21}: {alpha * 100}%")
    st.write(f"- {'Null Hypothesis':21}: The samples are uncorrelated")
    st.write(f"- {'Alternate Hypothesis':21}: The samples are correlated")
    st.write(f"- {'Test Statistic':21}: {statistic}")
    st.write(f"- {'p-value':21}: {pvalue}")
    if pvalue < alpha:
        st.write(f"- Since p-value is less than {alpha}, we Reject the Null Hypothesis at {alpha * 100}% significance level")
        st.write(f"- **CONCLUSION**: The variables {var1} and {var2} are correlated")
    else:
        st.write(f"- Since p-value is greater than {alpha}, we Fail to Reject the Null Hypothesis at {alpha * 100}% significance level")
        st.write(f"- **CONCLUSION**: The variables {var1} and {var2} are uncorrelated")

    # Spearman test
    spearman = stats.spearmanr(temp[var1].values, temp[var2].values)
    pvalue = spearman.pvalue
    statistic = spearman.statistic
    st.markdown("### Spearman Test")
    st.write(f"- {'Significance Level':21}: {alpha * 100}%")
    st.write(f"- {'Null Hypothesis':21}: The samples are uncorrelated")
    st.write(f"- {'Alternate Hypothesis':21}: The samples are correlated")
    st.write(f"- {'Test Statistic':21}: {statistic}")
...
def num_univar_plots(data, var, bins=10, figsize=(15, 7)):
    st.markdown(f"## Univariate Analysis of {var}")
    st.markdown("")
    col = data.loc[:, var].copy()
...
def num_bivar_plots(data, var_x, var_y, figsize=(12, 4.5), scatter_kwargs=dict(), hexbin_kwargs=dict()):
    st.markdown(f"## Bi-variate Analysis between {var_x} and {var_y}")
    st.markdown("")
...
def cat_summary(data, var):
    ...
    st.markdown("### Meta-data:")
    st.write(f"{'Data Type':15}: {col.dtype}")
    st.write(f"{'Cardinality':15}: {col.nunique(dropna=True)} categories")
    st.write(f"{'Missing Data':15}: {col.isna().sum():,} rows ({col.isna().mean() * 100:.2f} %)")
    st.write(f"{'Available Data':15}: {col.count():,} / {len(col):,} rows")
...
def num_cat_hyp_testing(data, num_var, cat_var, alpha=0.05):
  ...
  if pvalue < alpha:
    st.write(f"- Since p-value is less than {alpha}, we Reject the Null Hypothesis at {alpha * 100}% significance level")
    st.write(f"- CONCLUSION: The variables {num_var} and {cat_var} are associated to each other")
  else:
    st.write(f"- Since p-value is greater than {alpha}, we Fail to Reject the Null Hypothesis at {alpha * 100}% significance level")
    st.write(f"- CONCLUSION: The variables {num_var} and {cat_var} are not associated to each other")
...
def num_cat_bivar_plots(data,
    ...
    st.markdown(f"## Bi-variate Analysis between {cat_var} and {num_var}")
    st.markdown("")
...
def cat_bivar_plots(data,
    ...
    st.markdown(f"## Bi-variate Analysis between {var1} and {var2}")
    st.markdown("")
...
def cramers_v(data, var1, var2):
    ...
    n = ct.sum().sum()
    return (chi2 / (n * (min(ct.shape) - 1))) ** 0.5
...
def hyp_cat_cat(data, var1, var2, alpha=0.05):
    ...
    pvalue = chi2.pvalue
    st.write(f"- {'Cramers V':21}: {cramers_v(data, var1, var2)}")
    st.write(f"- {'Significance Level':21}: {alpha * 100}%")
    st.write(f"- {'Null Hypothesis':21}: The samples are uncorrelated")
    st.write(f"- {'Alternate Hypothesis':21}: The samples are correlated")
    st.write(f"- {'Test Statistic':21}: {statistic}")
...
def get_iqr_outliers(data, var, band=1.5):
    ...
    st.markdown(f"### {var} - IQR Limits:")
    st.write(f"{'Lower Limit':12}: {lower_limit}")
    st.write(f"{'Upper Limit':12}: {upper_limit}")
...
if 'dataframe' in st.session_state:
    ...
    st.write(df.head(5))
    # Dropdowns for EDA
    st.sidebar.header("Select Columns for EDA")
